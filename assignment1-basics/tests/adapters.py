...
def run_linear(
    ...
    layer = Linear(d_in, d_out, dtype=weights.dtype, device=weights.device)
...
def run_embedding(
    ...
    layer = Embedding(vocab_size, d_model, dtype=weights.dtype, device=weights.device)
    layer.load_state_dict({"weight": weights})
...
def run_swiglu(
    ...
    layer = SwiGLU(d_model, d_ff, dtype=in_features.dtype, device=in_features.device)
...
def run_rmsnorm(
    ...
    layer = RMSNorm(
        d_model, eps=eps, dtype=in_features.dtype, device=in_features.device
    )
    layer.load_state_dict({"weight": weights})
    return layer(in_features)
